...
'require ui';
'require rpc';
'require bs-menutree as tree';

// Track setInterval to prevent background timer leaks across SPA navigations
const _viewIntervals = (window.__bsViewIntervals || (window.__bsViewIntervals = new Map()));
(function hookIntervals() {
	if (window.__bsIntervalsHooked) return;
	window.__bsIntervalsHooked = true;
...
function flushUciCache() {
	...
	try { uci.unload(names); }
		...
		console.error('bs-router: uci.unload threw during cache flush', e);
		return null;
	}
	...
	return uci.load(refill).catch((e) => {
		console.error('bs-router: reloading uci ' + refill.join(', ') + ' failed', e);
	});
}
...
function renderedIn(view) {
	...
	return new Promise((resolve, reject) => {
		const timer = window.setTimeout(() => {
			finish(() => reject(new Error('bs-router: view did not render within ' + RENDER_TIMEOUT + ' ms')));
		}, RENDER_TIMEOUT);
...
function progressBar() {
	let bar = document.getElementById('bs-nav-progress');
	if (!bar) {
		bar = document.createElement('div');
		bar.id = 'bs-nav-progress';
		bar.setAttribute('aria-hidden', 'true');
		bar.style.cssText = 'position: fixed; top: 0; left: 0; right: 0; height: 3px; z-index: 99999; background: #0088cc; opacity: 0; transition: opacity 0.2s, transform 0.3s ease-out; transform: scaleX(0); transform-origin: left; pointer-events: none; box-shadow: 0 0 8px rgba(0, 136, 204, 0.6);';
...
function adoptEntry() {
	...
	if (st && st.bsid) { _curId = st.bsid; return; }
	_curId = newEntryId();
	try { history.replaceState(Object.assign({}, st, { bsid: _curId }), '', window.location.href); } catch (e) {}
}
...
function titleHost() {
	if (_titleHost === null) {
		const board = document.querySelector('header .brand');
		_titleHost = (board ? board.textContent.trim() : (document.title.split('|')[0] || '').trim()) || 'OpenWrt';
	}
	return _titleHost;
}
...
function navigate(pathname, push) {
	...
	const contentHost = document.querySelector('#maincontent');
	...
	if (push) {
		...
		history[same ? 'replaceState' : 'pushState']({ bsnav: true, bsid: _curId }, '', pathname);
	}
	...
	Promise.all([ warmedThen(className), previous.catch(() => {}) ]).then(() => {
		...
		console.error('bs-router: SPA nav to ' + className + ' failed, falling back to full load', e);
